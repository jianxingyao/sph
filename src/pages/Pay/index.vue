...
import QRCode from 'qrcode'
  ...
  export default {
    ...
    methods: {
      //获取支付订单信息
      async getData(){
        const result = await this.$API.requestPayOrderInfo(this.orderId)
        if(result.code === 200){
      ...
      async open() {
        ...
        this.$alert(`<img src=${url} >`, '微信扫一扫支付', {
        ...
        if(!this.timer){
          this.timer = setInterval(async () => {
            const result = await this.$API.requestOrderState(this.orderId)
            //支付成功
            if(result.code === 200){
              //记录请求状态码
