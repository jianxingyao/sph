...
export default {
  ...
  computed: {
    ...mapState("Trade", ["address", "tradeList"]),
    ...mapGetters("Trade", ["detailArrayList"]),
    //选中的收获地址
    userSite() {
      return (this.address.filter((item) => item.isDefault == "1")[0] || {})
        .fullAddress;
    },
    //选中收货人姓名
    userName() {
  ...
  methods: {
    ...
    async sumbitOrder() {
      ...
      const data = {
        ...
        consigneeTel: this.userPhoneNum,
        deliveryAddress: this.userSite,
        paymentWay: "ONLINE",
        orderComment: this.userMessage,
        orderDetailList: this.detailArrayList
      };
      
      const result =  await this.$API.reuqestOrderSumbit(tradeNo,data);
      if(result.code === 200){
        //存储订单号
  ...
  mounted() {
    this.$store.dispatch("Trade/getSiteInfo");
