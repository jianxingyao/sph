      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd 
                  ...
                  @click="changActive(spuSaleAttrValue,item.spuSaleAttrValueList)"
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNumber" @blur="changeSkuNumber"/>
                <a href="javascript:" class="plus">+</a>
                <a href="javascript:" class="mins">-</a>
              </div>
              <div class="add">
                <a href="javascript:" @click="addOrUpdateGoShopping">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods:{
    //让当前点击的盒子有 样式
    changActive(spuSaleAttrValue,spuSaleAttrValueList){
      spuSaleAttrValueList.forEach(t => {
        t.isChecked = '0'
      }) 
    ...
    //判断用户输入的数量是否 合法合理
    changeSkuNumber(e){
      let skuNumber = this.skuNumber * 1
      //不合法  改成 1 合法 取整数
      if(isNaN(skuNumber) || skuNumber < 1){
        this.skuNumber = 1
      }else{
        this.skuNumber = parseInt(skuNumber)
      }
    },
    //添加商品到购物车
    async addOrUpdateGoShopping(){
      try {
        await this.$store.dispatch('Detail/addOrUpdateGoShopping',{
  ...
  created(){
    this.$store.dispatch('Detail/getGoodsDetailsInfo',this.$route.params.skuid)
  }
};
</script>
